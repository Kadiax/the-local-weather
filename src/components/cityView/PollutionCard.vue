  ...
  <div class="pollution-card">
    ...
    <div class="polluant" v-for="(value, name) in polluantComponents" :key="name">
      ...
      <progress
        ...
        max="100"
        :class="polluantColorClass(name, value)"></progress>
...
<script setup lang="ts">
  ...
  import type { PolluantData } from '@/models/WeatherData';

  const props = defineProps<{
    polluantData: PolluantData;
  }>();

  // **Polluant Data Mapping**
  const polluantDetails: Record<string, { fullName: string; thresholds: Record<string, number> }> = {
    co: {
  ...
  // **Computed Properties**
  const polluantComponents = computed(() => props.polluantData?.list?.[0]?.components || {});

  // **Format Polluant Name**
  const formattedPolluantName = (name: string) => capitalize(name.replace('_', '.'));

  // **Calculate Progress Value**
  const polluantProgress = (name: string, value: number) =>
    polluantDetails[name] ? (value / polluantDetails[name].thresholds.poor) * 100 : 0;
